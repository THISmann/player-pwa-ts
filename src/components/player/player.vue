...
const getRadioByName = async (radioName: string) => {
  try {
    ...
    const response = await axios.get(
      `http://localhost:8030/api/radios/${radioName}`,
      {
        headers: {
...
<template>
  <div>
    <div
      class="container-fluid h-screen bg-fixed m-0 p-0 containerRadio"
    ...
    >
      <div class="container-fluid max-md:hidden h-screen">
        ...
        <div class="grid grid-cols-12">
          <div class="col-span-9 bg-slate-100/25 rounded-lg m-1">
            <div
              v-for="advert in advertLists"
              :key="advert.id"
              v-show="advert === currentAdvert"
              class="p-1 bg-gray-200 rounded-lg m-3 shadow-2xl"
            >
              <img
                :src="advert.pub_image ? advert.pub_image : './assets/pub.jpg'"
          ...
          <div class="col-span-3 bg-slate-100/25 rounded-lg m-1">
            <div class="m-6 p-1 rounded-lg bg-gray-200">
              ...
              <img
                :src="currentTrack.img_medium_url || img || icecastImgUrl"
                alt=""
                class="w-full h-32 rounded-lg"
              />
            </div>
        ...
        <div class="grid grid-cols-12 bg-slate-100/25 rounded-lg m-1">
            ...
            <div class="flex p-1">
              <div id="cardPlayer" class="p-1 m-1 bg-gray-200 rounded-lg">
                <div>
                  ...
                  <img
                    :src="currentTrack.img_medium_url || img || icecastImgUrl"
                    class="max-md:w-14 md:w-20 md:h-20 max-md:h-14"
                    alt=""
                    srcset=""
                  />
                </div>
          ...
          <div class="col-span-6 flex-col items-center justify-center">
            <div class="mt-7">
              <div
                v-if="!play"
                class="mt-2 h-2 w-full bg-gray-200 dark:bg-gray-600"
              >
          ...
          >
            ...
            <div>
              ...
              <div class="flex" id="volumeControl">
                ...
                <input
                  class="bg-gray-200 rounded-lg appearance-none cursor-all-scroll dark:bg-white"
                  type="range"
                  v-model="volume"
                  @input="updateVolume"
                  min="0"
      ...
      <div class="md:hidden container-fluid h-screen">
        <!-- <h1> mobile </h1> -->
        <div
          class="mx-11 mt-4 p-1 rounded-lg bg-gradient-to-r from-gray-200/25 to-gray-100 w-72 h-80"
        >
          <div class="m-5 h-72">
            ...
            <img
              :src="icecastImgUrl || img"
              v-if="server_type === 'icecast'"
              alt=""
              class="w-full h-60 rounded-lg"
            />
            <img
              :src="icecastImgUrl || img"
              v-if="server_type === 'shoutcast'"
              alt=""
              class="w-full h-60 rounded-lg"
            />
            <img
              :src="currentTrack.img_medium_url || img"
              v-if="
                server_type === 'everestcast' ||
                server_type === 'rcast' ||
                server_type === 'centovacast' ||
        ...
        <div class="mx-11">
          ...
          <div
            v-if="!playMobile"
            class="mt-2 h-1 w-full bg-gray-400 dark:bg-neutral-600"
          >
        ...
        <div class="p-1 mx-7 overflow-y-auto">
          <div
            class="flex sm:w-full sm:h-28 border-black-300/75 rounded-lg shadow-2xl p-1 m-1 scroll"
            v-for="data in historyTrack"
            :key="data.id"
          >
            <img
