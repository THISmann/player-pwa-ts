...
//     currentAdIndex.value = (currentAdIndex.value + 1) % advertStore.advertLists.length;
//     currentAdvert.value = advertStore.advertLists[currentAdIndex.value];
// };
...
//             advertStore.advertLists = response.data.results;
//         })
//     } catch (error) {
//         alert(error);
//         if (axios.isAxiosError(error)) {
    ...
    <div :style="{ 'background-color': currentBgColor || '#FF6503' }">
        <div class="container-fluid  max-md:hidden h-screen">
            ...
            <div class="grid grid-cols-12">
                <div class="col-span-9  bg-slate-100/25 rounded-lg m-1">
                    <!-- 
                    <div v-for="advert in advertStore.advertLists" :key="advert.id" v-show="advert === currentAdvert"
                        class="p-1 bg-gray-200 rounded-lg m-3 shadow-2xl">
                        <img :src="advert.pub_image ? advert.pub_image : './assets/pub.jpg'" class="h-80 w-full rounded-lg"
                            alt="" srcset="">
                    ...
                    <div id="cards-section" class="">
                        ...
                        <div class="flex p-1 mx-7">
                            <div v-for="data in historyTrack" :key="data.id"
                                ...
                                <h1 class="text-left mx-1 text-md text-gray-100 bg-gray-500/25 p-1 mt-1 rounded-lg ">
                                    {{ data.title }} </h1>
                            </div>
                        </div>
        ...
        currentTrack.author }} </h4>
                        ...
                        <div v-for="serverType in serverTypes" :key="serverType">
                            <h4 v-if="server_type === '' || server_type === serverType && serverType === 'icecast'"
                                class="text-white text-left m-1">
                                {{
        currentTrack.server_description === null ? 'default' : currentTrack.server_description
    }}
                            </h4>
                        ...
                        <div class="p-4">
                            ...
                            <!-- 
                                <h3 v-if="server_type === serverType && serverType === 'icecast'"
                                    class="text-gray-200 text-md text-left"> {{ currentTrack.server_description }}</h3>
        ...
        <div class="md:hidden container-fluid h-screen">
            ...
            <div class="flex p-1 mx-7 overflow-x-auto">
                <div class="w-28 h-28 border-black-300/75 rounded-lg shadow-2xl p-1 m-1" v-for="data in historyTrack"
                    :key="data.id">
