...
export const fetchAndProcessRadioData = async (url: string) => {
  ...
  try {
    ...
    const data =
      localStorage.getItem("playerServerType") !== "shoutcast"
        ? await response.json()
        : null;
    ...
    switch (localStorage.getItem("playerServerType")) {
      case "icecast":
        currentTrack = JSON.parse(
          JSON.stringify(data?.icestats.source[0], null, 2)
        );
      ...
      case "centovacast":
        currentTrack = {
          ...
          author: JSON.parse(
            JSON.stringify(data?.data[0].track.artist, null, 2)
          ),
          img_medium_url: JSON.parse(
            JSON.stringify(data?.data[0].track.imageurl, null, 2)
          ),
        };
        break;
      case "azuracast":
        currentTrack = {
          title: JSON.parse(
            JSON.stringify(data?.now_playing.song.title, null, 2)
          ),
          album: JSON.parse(
            JSON.stringify(data?.now_playing.song.album, null, 2)
          ),
          author: JSON.parse(
            JSON.stringify(data?.now_playing.song.artist, null, 2)
          ),
          img_medium_url: JSON.parse(
            JSON.stringify(data?.now_playing.song.art, null, 2)
          ),
        };
        // console.log('++++++', currentTrack);
        break;
      case "radioking":
        currentTrack = {
          ...
          img_medium_url: JSON.parse(
            JSON.stringify(data[0].cover_url, null, 2)
          ),
        };
        break;
      case "everestcast":
        currentTrack = {
          ...
          img_medium_url: data.results[0].img_medium_url,
        };
        console.log("________+++", currentTrack);
    ...
    return (currentTrack = {
      title: "bloque par cors",
      album: "bloque par cors",
      author: "bloque par cors",
      img_medium_url: "",
    });
  }
};
